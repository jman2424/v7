...
def start(data):
    ...
    with session_store.connection() as db:
        ...
        db.execute("UPDATE registration_requests SET password_hash='',code_hash='',status='expired' WHERE status='verification' AND expires<?", (now,))
        db.execute('DELETE FROM registration_requests WHERE created<?', (now-30*86400,))
        recent = db.execute('SELECT COUNT(*),MAX(created) FROM registration_requests WHERE email=? AND created>?', (email, now-86400)).fetchone()
        if recent[0] >= 5 or (recent[1] and recent[1] > now-60):
            raise ValueError('Please wait before requesting another verification code.')
        db.execute('INSERT INTO registration_requests (id,email,password_hash,kind,tenant,business_name,code_hash,expires,status,created) VALUES (?,?,?,?,?,?,?,?,?,?)',
...
def status():
    with session_store.connection() as db:
        _schema(db)
        db.execute("UPDATE registration_requests SET password_hash='',code_hash='',status='expired' WHERE (status='verification' AND expires<?) OR (status='pending' AND created<?)", (time.time(),time.time()-30*86400))
        row = db.execute('SELECT email,tenant,status,expires,attempts,created FROM registration_requests WHERE id=?', (session.get('registration_request', ''),)).fetchone()
    if not row:
        return None
    state = row[2]
...
def confirm(code):
    ...
    with session_store.connection() as db:
        _schema(db)
        db.execute('BEGIN IMMEDIATE')
        row = db.execute("SELECT email,password_hash,kind,tenant,business_name,code_hash FROM registration_requests WHERE id=? AND status='verification' AND expires>? AND attempts<5", (request_id,time.time())).fetchone()
        if not row:
        ...
        if not hmac.compare_digest(expected, _code_hash(request_id,code)):
            db.execute('UPDATE registration_requests SET attempts=attempts+1 WHERE id=?', (request_id,))
            db.execute("UPDATE registration_requests SET status='expired',password_hash='',code_hash='' WHERE id=? AND attempts>=5", (request_id,))
            invalid = True
        else:
            ...
            if kind == 'join':
                ...
                db.execute("UPDATE registration_requests SET status='pending',code_hash='' WHERE id=?", (request_id,))
            else:
                owner_details = (email,password_hash,tenant,name)
                db.execute("UPDATE registration_requests SET status='creating',code_hash='' WHERE id=?", (request_id,))
    if owner_details:
        ...
        try:
            ...
            with session_store.connection() as db:
                db.execute("UPDATE registration_requests SET status='failed',password_hash='' WHERE id=?", (request_id,))
        ...
        with session_store.connection() as db:
            db.execute("UPDATE registration_requests SET status='approved',password_hash='' WHERE id=?", (request_id,))
    if invalid:
...
def pending(tenant):
    with session_store.connection() as db:
        _schema(db)
        rows = db.execute("SELECT id,email,created FROM registration_requests WHERE tenant=? AND status='pending' AND created>? ORDER BY created LIMIT 100", (tenant,time.time()-30*86400)).fetchall()
...
def decide(tenant, request_id, approve, actor):
    with session_store.connection() as db:
        _schema(db)
        db.execute('BEGIN IMMEDIATE')
        row = db.execute("SELECT email,password_hash FROM registration_requests WHERE id=? AND tenant=? AND status='pending' AND created>?", (request_id,tenant,time.time()-30*86400)).fetchone()
        if not row:
        ...
        db.execute("UPDATE registration_requests SET status=?,password_hash='',code_hash='',decided_by=? WHERE id=?", ('approved' if approve else 'rejected',actor,request_id))
