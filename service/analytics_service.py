...
def _norm_session_id(sid: Optional[str]) -> str:
    return (sid or "unknown").strip() or "unknown"
...
class AnalyticsService:
    ...
    def ensure_ready(self) -> None:
        """
        Migration-safe schema that matches dashboard expectations.
        ...
        with self._conn() as con:
            ...
            # Ensure columns exist even if older schema created the tables
            _ensure_columns(
                con,
                "events",
                {
    ...
    def log_message(
        ...
        *,
        ...
        is_error: bool = False,  # legacy flag
        error_code: str = "",
        error_type: str = "",
        ...
        with self._conn() as con:
            con.execute(
                ...
                (
                    ...
                    sid,
                    event_type,
                    (intent or "unknown").strip().lower() or "unknown",
                    text or "",
                    (lead_id or "").strip(),
                    error_code or "",
                    error_type or "",
    ...
    def log_event(self, *args: Any, **kwargs: Any) -> None:
        ...
        try:
            ...
            session_id = _norm_session_id(kwargs.get("session_id") or (args[2] if len(args) > 2 else "unknown"))
            event_type = str(kwargs.get("event_type") or (args[3] if len(args) > 3 else "event"))
            lead_id = str(kwargs.get("lead_id") or (args[4] if len(args) > 4 else "")) if True else ""
            ...
            with self._conn() as con:
                con.execute(
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?);
                    """,
                    (_utc_now_iso(), tenant, channel, session_id, event_type, lead_id, json.dumps(meta_json) if isinstance(meta_json, dict) else str(meta_json or "")),
                )
        except Exception:
            return
    ...
    def get_overview_daily(self, *, tenant: str, minutes: int = 1440, limit_days: int = 45) -> List[Dict[str, Any]]:
        ...
        tenant_n = _norm_tenant(tenant)
        since = _since_iso(minutes)
        limit_days = _clamp_int(limit_days, 45, 1, 365)
        ...
        # reverse for ascending chart order
        out: List[Dict[str, Any]] = []
    ...
    def leads_csv(self, *, tenant: str) -> str:
        tenant_n = _norm_tenant(tenant)
        rows = self.get_leads(tenant=tenant_n, limit=500)
