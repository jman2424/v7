...
<section class="billing" aria-label="Subscription and payments">
  ...
  {#if data}
    ...
    <div class="cards">
      <article class="panel"><h3>Platform subscription</h3><strong class="price">£400 <small>/ month before VAT</small></strong><p>£80 VAT · <b>£480/month total</b></p><p>Status: {baseContract?.status||'Not subscribed'}</p><p>Next payment: {baseContract?.cancel_at_end?'Renewal cancelled':date(baseContract?.next_due)}</p>{#if !baseContract||['not_started','canceled','incomplete_expired'].includes(baseContract.status)}<button disabled={busy||!data.configured} on:click={()=>action('checkout',{kind:'platform'})}>Subscribe with Stripe</button>{/if}</article>
      <article class="panel"><h3>Implementation</h3><strong class="price">£200 <small>one time before VAT</small></strong><p>£40 VAT · <b>£240 total</b></p><p>{baseContract?.implementation_paid?'Paid on the first subscription invoice.':'Added to the first platform subscription invoice.'}</p><p>First platform payment including implementation: <b>£720</b>.</p></article>
      <article class="panel"><h3>WhatsApp add-on</h3><strong class="price">£200 <small>/ month before VAT</small></strong><p>£40 VAT · <b>£240/month total</b></p><p>{waContract?(waContract.paused?'Deactivated':waContract.status==='active'?'Active':'Payment pending'):'Not subscribed'}</p><p>{waContract?.cancel_at_end?'Paid-through date: ':'Next payment: '}{date(waContract?.next_due)}</p>
        ...
        {:else}<button disabled={busy||!data.configured||baseContract?.status!=='active'} on:click={()=>action('checkout',{kind:'whatsapp'})}>Activate through Stripe</button>{/if}
    ...
    <article class="panel"><header><div><h2>Payment history</h2><p>Previous subscription, implementation, WhatsApp and API invoices. Showing the latest 120 recorded invoices.</p></div><label>Show<select bind:value={invoiceKind} on:change={()=>invoicePage=1}><option value="all">All charges</option><option value="platform">Platform &amp; implementation</option><option value="whatsapp">WhatsApp</option><option value="api">API usage</option></select></label></header>
    ...
    <article class="panel"><h2>Monthly API costs</h2><p>Usage estimates and approved charges are different. Estimates use recorded model usage and an exchange rate; missing usage is not treated as free. Your platform administrator reviews a completed month before billing it.</p>
