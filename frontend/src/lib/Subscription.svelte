...
<section class="billing" aria-label="Subscription and payments">
  ...
  {#if data}
    ...
    <div class="cards"><article class="panel"><h3>Total paid</h3><strong class="price">{money(data.totals.paid)}</strong><p>Recorded invoice payments, before refunds.</p></article><article class="panel"><h3>Total payment due</h3><strong class="price">{money(data.totals.due+data.totals.approved_api_due)}</strong><p>Open invoices including VAT.{#if canViewCosts} Approved API charges awaiting checkout: {money(data.totals.approved_api_due)}.{/if}</p></article><article class="panel"><h3>Regular monthly price</h3><strong class="price">{baseContract?.status==='active'?money(48000+(waContract?.status==='active'&&!waContract.paused?24000:0)):'Not subscribed'}</strong><p>Including VAT; API usage is additional.</p></article></div>
